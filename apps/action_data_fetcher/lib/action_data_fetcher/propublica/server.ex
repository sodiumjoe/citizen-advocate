defmodule ActionDataFetcher.Propublica.Server do
  use GenServer

  @timeout Application.get_env(:action_data_fetcher, :timeouts)[:propublica]
  ...
  def handle_call({:fetch_member_data}, _from, state) do
    tasks = Enum.map(@chambers, &queue_fetch(&1))
  ...
  def handle_info({:start_fetcher_pool}, state) do
    ActionDataFetcher.Propublica.Supervisor.start_fetcher_pool
    {:noreply, state}
  end

  ## Internal Helpers

  defp queue_fetch(chamber) do
    Task.async(
      fn -> :poolboy.transaction(:propublica_fetchers,
        &(GenServer.call(&1, {:fetch_members, { :congress, @congress, :chamber, chamber}}, @timeout)
      ), @timeout)
    end)
  end
